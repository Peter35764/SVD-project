...
struct MetricSetting {
  ...
  MetricSetting(MetricType t, const std::string &n, bool rel, T p_val, bool en)
      : type(t), name(n), relative(rel), p(p_val), enabled(en) {
    if (!(p_val > T(0))) {
...
void svd_test_func(std::string fileName,
                   ...
                   const int n, const std::string &algorithmName,
                   int lineNumber,
...
int main() {
  ...
  std::vector<double> sigmaRatios =
      sigma_ratio;  // macro sigma_ratio: {1.01, 1.2, 2, 8, 30, 100}
  std::vector<std::pair<int, int>> matrixSizes =
      matrix_size;  // macro matrix_size: {{5, 5}}
  ...
  std::vector<MetricSetting<double>> metricsSettings = {
      ...
      MetricSetting<double>(MetricType::MAX_DEVIATION,
                            ...
                            true),
      MetricSetting<double>(MetricType::ERROR_SIGMA, "rel avg err sigma", true,
                            0.7, true),
      ...
      MetricSetting<double>(
          ...
          true)};  //     bool relative;      // true: относительная ошибка,
                   //     false: абсолютная
  // генерируется таблица в файле "jacobi_test_table.txt" теста метода
  // Eigen::JacobiSVD с соотношением сингулярных чисел:  1.01, 1.2, 2, 5, 10, 50
  // ---    6 причем каждое соотношение относится к двум интервалам сингулярных
  // чисел:
  //                       маленьких {0,1}, больших {1,100} (это не
  //                       параметризованно)   ---   2
  // с матрицами размеров: {3,3}, {5,5}, {10,10}, {20,20}, {50,50}, {100,100}
  // ---   6 6*2*6 = 72 - всего столько строк будет в таблице размер выборки для
  // усреднения: 20
