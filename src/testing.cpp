...
template<typename T, template<typename> class gen_cl, template<typename, typename> class svd_cl>
void svd_test_func_reverse_jacobi(
    std::string fileName,
    const std::vector<T> &SigmaMaxMinRatiosVec,
    const std::vector<std::pair<int, int>> &MatSizesVec,
    const int n,
        ...
        for (const auto &SigmaMaxMinRatio : SigmaMaxMinRatiosVec) {
            for (const auto &interval : Intervals) {
                ...
                for (int i = 1; i <= n; ++i) {
                    ...
                    S_true = svd_gen.MatrixS();
                    V_true = svd_gen.MatrixV();
                    svd_cl<MatrixDynamic, VectorDynamic> svd_func(
                        (U_true * S_true * V_true.transpose()).eval(),
                        S_true,
                        Eigen::ComputeFullU | Eigen::ComputeFullV);
                    U_calc = svd_func.matrixU();
...
int main()
{
    ...
    std::thread t3([&]() {
        ...
        auto t_start = std::chrono::high_resolution_clock::now();
        svd_test_func_reverse_jacobi<double, SVDGenerator, SVD_Project::RevJac_SVD>(
            file_name,
            sigma_ratio,
            matrix_size,
            matrix_num_for_sample_averaging,
    ...
    // Перемещаем курсор на  для вывода итогового времени
    std::cout << "\033[5;0H";
