...
// Назначение функции - тестирование и оценка точности методов SVD разложения,
// унаследованных от SVD Base
...
/*
 ...
 Размер выборки фиксированного размера матриц определяется задаваемым параметром
 'n'.
*/

/*
Функция написана с теми условиями, что:
    1. Класс SVD разложения наследуется от класса Eigen::SVDBase, причем должен
существовать конструктор класса, который вторым параметром принимает настройку
вычислений матриц U и V, т.е. thin или full.
    2. Генерация случайных матриц происходит с помощью SVDGenerator из
generate_svd.h
    3. В функцию передаётся std::vector соотношений максимумального и
минимального сингулярного числа
    4. В функцию передаётся std::vector<std::pair<int,int>> размеров матриц для
исследования
    5. В функцию передаётся int n размер выборки фиксированного размера матриц
для подсчёта средних
    6. Функция работает достаточно долго, особенно для матриц больших размеров,
поэтому выводится прогресс в процентах
...
- SigmaMaxMinRatiosVec: вектор сооотношения максимального и минимального
сингулярных чисел; нужен т.к. ошибка может сильно отличаться у разных
...
усреднения выборки и подсчета средних
*/
template <typename T, template <typename> class gen_cl,
          template <typename> class svd_cl>
void svd_test_func(std::string fileName,
    ...
    // лямбда-функция печати таблицы, вторым параметром принимает матрицу
    // строк(так реализована таблица)
    auto printTable = [](std::ostream &out,
    ...
    // больше размер матрицы -> дольше будет вычисляться;
    T generalSum = 0;
    ...
    // интервалы, в которых будут генерироваться сингулярные числа для теста на
    // малых по модулю сингулярных числах и больших
    const std::vector<std::pair<T, T>> Intervals = {{0, 1}, {1, 100}};

    // сам генератор чисел
    std::random_device rd; // случайное число - значение сида
    std::default_random_engine gen(
        rd()); // генерация последовательности псевдослучайных цифр

    // название столбцов таблицы
    std::vector<std::vector<std::string>> table = {
        {"Dimension", "Sigma-max/min-ratio", "SV interval", "AVG ||I-U_t*U||",
         "AVG ||I-U*U_t||", "AVG ||I-V_t*V||", "AVG ||I-V*V_t||",
         "AVG relative err. sigma"}};

    T ProgressCoeff = n * Intervals.size() * SigmaMaxMinRatiosVec.size() *
                      generalSum / 100.0; // наибольшее значение прогресса
    T progress = 0; // инициализация аккумулятора прогресса

    MatrixDynamic U_true, S_true, V_true, U_calc, V_calc, V_calc_transpose;
    VectorDynamic
        SV_calc; // аналог S_true, но не матрица, а вектор сингулярных значений
    ...
    for (const auto &MatSize : MatSizesVec) {
        ...
        for (const auto &SigmaMaxMinRatio : SigmaMaxMinRatiosVec) {
            for (const auto &interval : Intervals) {
                ...
                /*
                    проверка на то, что отношение sigmaMax/sigmaMin <= b/a, где
                   a и b - границы интервала, очевидно, что если неравенство не
                   выполняется, то сингулярных чисел с заданным соотношением и в
                   заданном интервале не существует
                */
                assert((interval.first * SigmaMaxMinRatio <= interval.second,
                        "\nError: no sigma values exist with such ratio in "
                        "such interval\n"));
                ...
                */
                ...
                // заполнение вектора истинных сингулярных чисел из уже
                // определенного диапазона:
                std::uniform_real_distribution<T> distr(sigma_min, sigma_max);
                assert((minNM >= 2, "\nError: no columns or rows allowed\n"));

                // инициализация аккумуляторов выводимых в таблицу значений
                // характеристик
                T avg_dev_UUt = 0, avg_dev_UtU = 0, avg_dev_VVt = 0,
                  avg_dev_VtV = 0, avg_relErr_sigma = 0;

                // цикл для усреднения результатов(выборка из n матриц с
                // одинаковыми параметрами)
                for (int i = 1; i <= n; ++i) {
                    gen_cl<T> svd_gen(
                        N, M, gen, distr,
                        true); // инициализация параметров генерации
                    ...
                    avg_dev_UUt += (MatrixDynamic::Identity(N, N) -
                                    U_calc * U_calc.transpose())
                                       .squaredNorm() /
                                   n;
                    avg_dev_UtU += (MatrixDynamic::Identity(N, N) -
                                    U_calc.transpose() * U_calc)
                                       .squaredNorm() /
                                   n;
                    avg_dev_VVt += (MatrixDynamic::Identity(M, M) -
                                    V_calc * V_calc.transpose())
                                       .squaredNorm() /
                                   n;
                    avg_dev_VtV += (MatrixDynamic::Identity(M, M) -
                                    V_calc.transpose() * V_calc)
                                       .squaredNorm() /
                                   n;
                ...
                table.emplace_back(std::vector<std::string>{
                    num2str(N) + "x" + num2str(M), num2str(SigmaMaxMinRatio),
                    "[" + num2str(interval.first) + ", " +
...
// Wrapper to make GivRef_SVD compatible with the testing framework
namespace SVD_Project {
template <typename Matrix> struct GivRef_SVD_Wrapper {
    typedef Matrix MatrixType;

    GivRef_SVD_Wrapper(const MatrixType &matrix,
                       unsigned int computationOptions = 0)
        : m_svd(matrix, computationOptions) {}

    const auto &matrixU() const { return m_svd.matrixU(); }
    const auto &matrixV() const { return m_svd.matrixV(); }
    const auto &singularValues() const { return m_svd.singularValues(); }

  private:
    GivRef_SVD<MatrixType> m_svd;
};
} // namespace SVD_Project

// Template specialization for the wrapper
namespace Eigen {
namespace internal {
template <typename MatrixType>
struct traits<SVD_Project::GivRef_SVD_Wrapper<MatrixType>>
...
int main() {
    ...
    // генерируеся таблицу в файле "jacobi_test_table.txt" теста метода
    // Eigen::JacobiSVD с соотношением сингулярных чисел:  1.01, 1.2, 2, 5, 10,
    // 50      ---    6 причем каждое соотношение относится к двум интервалам
    // сингулярных чисел:
    ...
    svd_test_func<double, SVDGenerator, Eigen::JacobiSVD>(
        "jacobi_test_table.txt", {1.01, 1.2, 2, 5, 10, 50},
        {{3, 3}, {5, 5}, {10, 10}, {20, 20}, {50, 50}, {100, 100}}, 20);
    svd_test_func<double, SVDGenerator, DQDS_SVD>(
        "dqds_test_table.txt", {1.01, 1.2, 2, 5, 10, 50},
        {{3, 3}, {5, 5}, {10, 10}, {20, 20}, {50, 50}, {100, 100}}, 20);
    svd_test_func<double, SVDGenerator, SVD_Project::GivRef_SVD_Wrapper>(
        "givens_refinement_results.txt", {1.01, 1.2, 2, 5, 10, 50},
        {{3, 3}, {5, 5}, {10, 10}, {20, 20}, {50, 50}, {100, 100}}, 20);
