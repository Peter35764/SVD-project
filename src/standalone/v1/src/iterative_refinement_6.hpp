...
template<typename T>
AOIR_SVD_6<T>::AOIR_SVD_6(const Matrix<T>& A, const std::string& history_filename_base) {
    ...
    AOIR_SVD_6<T> result = MSVD_SVD(A, initial_U, initial_V, history_filename_base);
...
template<typename T> void AOIR_SVD_6<T>::Set_U(const Matrix<T>& U_in) { this->U = U_in; }
template<typename T> void AOIR_SVD_6<T>::Set_V(const Matrix<T>& V_in) { this->V = V_in; }

template<typename T> void AOIR_SVD_6<T>::Set_S(const Matrix<T>& Sigma_full) { this->S = Sigma_full; }
template<typename T> Matrix<T> AOIR_SVD_6<T>::matrixU() const { return this->U; }
template<typename T> Matrix<T> AOIR_SVD_6<T>::matrixV() const { return this->V; }
template<typename T> Matrix<T> AOIR_SVD_6<T>::singularValues() const { return this->S; }
...
template<typename T>
AOIR_SVD_6<T> AOIR_SVD_6<T>::MSVD_SVD(
    const Matrix<T>& A,
    ...
    if (log_history) {
        history_filename = history_filename_base + "_conv6_full.csv";
    ...
    while (true) {
        ...
        Matrix<T> R = R_full.topLeftCorner(n, n);


        Matrix<T> S_algo = S_full_ortho;
        ...
        Cb.noalias() = Ca.transpose() - Sigma_n * R + S_algo * Sigma_n;
        ...
        Matrix<T> G = Matrix<T>::Zero(n, n); Matrix<T> F11 = Matrix<T>::Zero(n, n);

        for (int i = 0; i < n; ++i) {
             G(i, i) = S_algo(i, i) / T("2.0");
             F11(i, i) = R(i, i) / T("2.0");
        ...
        Matrix<T> F = Matrix<T>::Zero(m, m); F.topLeftCorner(n, n) = F11;
