...
class AOIRWrapper_5 {
public:
    AOIRWrapper_5(const MatrixType& A,
...
private:
    SVD_Project::AOIR_SVD_5<typename MatrixType::Scalar> impl;
};
...
int main() {
    ...
    std::string algo_name_5 = "Iterative Refinement 5 SVD (float128)";
    std::string file_name_5 = "iterative_refinement_5_float128_table.txt";
    auto t_start_5 = std::chrono::high_resolution_clock::now();



    svd_test_func<float128, SVDGenerator, AOIRWrapper_5>(file_name_5,
        sigma_ratio_vec, matrix_size_vec,
        matrix_num_for_sample_averaging, algo_name_5, next_line_for_progress++);

    auto t_end_5 = std::chrono::high_resolution_clock::now();
    double duration_algo_5 = std::chrono::duration<double>(t_end_5 - t_start_5).count();
    test_times.emplace_back(algo_name_5, duration_algo_5);
    ...
    std::cout << "\nExecution time for " << algo_name_5 << " = "
              << std::fixed << std::setprecision(4) << duration_algo_5 << " seconds.\n";
    ...
    if (timeFile) {
        timeFile << algo_name_5 << " : " << duration_algo_5 << " seconds\n";
