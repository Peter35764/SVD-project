...
template<typename T>
AOIR_SVD_1<T>::AOIR_SVD_1(const Matrix<T>& A, const std::string& history_filename_base) {
    ...
    AOIR_SVD_1<T> result = MSVD_SVD(A, initial_U, initial_V, history_filename_base);
...
template<typename T> void AOIR_SVD_1<T>::Set_U(const Matrix<T>& U_in) { this->U = U_in; }
template<typename T> void AOIR_SVD_1<T>::Set_V(const Matrix<T>& V_in) { this->V = V_in; }
template<typename T>
void AOIR_SVD_1<T>::Set_S(const Matrix<T>& Sigma_n_diag) {
...
template<typename T> Matrix<T> AOIR_SVD_1<T>::matrixU() const { return this->U; }
template<typename T> Matrix<T> AOIR_SVD_1<T>::matrixV() const { return this->V; }
template<typename T> Matrix<T> AOIR_SVD_1<T>::singularValues() const { return this->S; } 

template<typename T>
AOIR_SVD_1<T> AOIR_SVD_1<T>::MSVD_SVD(
    const Matrix<T>& A,
    ...
    if (log_history) {
        history_filename = history_filename_base + "_conv1_full.csv";
