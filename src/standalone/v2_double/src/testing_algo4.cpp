...
class AOIRWrapper_4 {
public:
    AOIRWrapper_4(const MatrixType& A,
...
private:
    SVD_Project::AOIR_SVD_4<typename MatrixType::Scalar> impl;
};
...
int main() {
    ...
    std::string algo_name_4 = "Iterative Refinement 4 SVD (float128)";
    std::string file_name_4 = "iterative_refinement_4_float128_table.txt";
    auto t_start_4 = std::chrono::high_resolution_clock::now();



    svd_test_func<float128, SVDGenerator, AOIRWrapper_4>(file_name_4,
        sigma_ratio_vec, matrix_size_vec,
        matrix_num_for_sample_averaging, algo_name_4, next_line_for_progress++);

    auto t_end_4 = std::chrono::high_resolution_clock::now();
    double duration_algo_4 = std::chrono::duration<double>(t_end_4 - t_start_4).count();
    test_times.emplace_back(algo_name_4, duration_algo_4);
    ...
    std::cout << "\nExecution time for " << algo_name_4 << " = "
              << std::fixed << std::setprecision(4) << duration_algo_4 << " seconds.\n";
    ...
    if (timeFile) {
        timeFile << algo_name_4 << " : " << duration_algo_4 << " seconds\n";
