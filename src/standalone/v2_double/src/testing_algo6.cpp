...
class AOIRWrapper_6 {
public:
    AOIRWrapper_6(const MatrixType& A,
...
private:
    SVD_Project::AOIR_SVD_6<typename MatrixType::Scalar> impl;
};
...
int main() {
    ...
    std::string algo_name_6 = "Iterative Refinement 6 SVD (float128)";
    std::string file_name_6 = "iterative_refinement_6_float128_table.txt";
    auto t_start_6 = std::chrono::high_resolution_clock::now();



    svd_test_func<float128, SVDGenerator, AOIRWrapper_6>(file_name_6,
        sigma_ratio_vec, matrix_size_vec,
        matrix_num_for_sample_averaging, algo_name_6, next_line_for_progress++);

    auto t_end_6 = std::chrono::high_resolution_clock::now();
    double duration_algo_6 = std::chrono::duration<double>(t_end_6 - t_start_6).count();
    test_times.emplace_back(algo_name_6, duration_algo_6);
    ...
    std::cout << "\nExecution time for " << algo_name_6 << " = "
              << std::fixed << std::setprecision(4) << duration_algo_6 << " seconds.\n";
    ...
    if (timeFile) {
        timeFile << algo_name_6 << " : " << duration_algo_6 << " seconds\n";
