...
class AOIRWrapper_1 {
public:
    AOIRWrapper_1(const MatrixType& A,
...
private:
    SVD_Project::AOIR_SVD_1<typename MatrixType::Scalar> impl;
};
...
int main() {
    ...
    std::string algo_name_1 = "Iterative Refinement 1 SVD (float128)";
    std::string file_name_1 = "iterative_refinement_1_float128_table.txt";
    auto t_start_1 = std::chrono::high_resolution_clock::now();



    svd_test_func<float128, SVDGenerator, AOIRWrapper_1>(file_name_1,
        sigma_ratio_vec, matrix_size_vec,
        matrix_num_for_sample_averaging, algo_name_1, next_line_for_progress++);

    auto t_end_1 = std::chrono::high_resolution_clock::now();
    double duration_algo_1 = std::chrono::duration<double>(t_end_1 - t_start_1).count();
    test_times.emplace_back(algo_name_1, duration_algo_1);
    ...
    std::cout << "\nExecution time for " << algo_name_1 << " = "
              << std::fixed << std::setprecision(4) << duration_algo_1 << " seconds.\n";
    ...
    if (timeFile) {
        timeFile << algo_name_1 << " : " << duration_algo_1 << " seconds\n";
