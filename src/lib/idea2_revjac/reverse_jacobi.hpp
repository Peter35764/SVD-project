...
RevJac_SVD<_MatrixType>& RevJac_SVD<_MatrixType>::Compute(
  ...
  traversalOrder.reserve(MAX_ITERATIONS * initial.rows() * (initial.cols() - 1) / 2);
  ...
  for (size_t iter = 0; iter < MAX_ITERATIONS; ++iter) {
    ...
    std::sort(traversalOrder.begin(), traversalOrder.end(), [](auto a, auto b) {
    ...
    for (const auto& [indices, _] : traversalOrder) {
        ...
        Index j = indices.second;
            ...
            for (const auto& rotType : {RotationType::Left, RotationType::Right}) {
                ...
                // regardless of the type of point: local maximum, global minimum and etc.
