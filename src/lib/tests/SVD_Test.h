...
/**
 * @brief Класс для тестирования различных алгоритмов сингулярного разложение
 * (SVD).
 *
 * Предоставляет фреймворк для генерации тестовых матриц, выполнения SVD с
 * разными алгоритмами, вычесление метрик и сравнения результатов.
 *
 * @tparam FloatingPoint Тип данных для плавающей точки (например, `float` или
 * `double`).
 * @tparam MatrixType Тип матрицы (например, `Eigen::Matrix<FloatingPoint,
...
class SVD_Test {
 public:
  ...
  enum MetricType {
    ...
    ERROR_SIGMA,  ///< Ошибка в сингулярных значениях : $|| \Sigma_{true} -
                  ///< \Sigma_{calc} ||_p$ (абсолютная) или $|| (\Sigma_{true} -
                  ///< \Sigma_{calc}) ./ \Sigma_{true} ||_p$ (относительная)
    RECON_ERROR,  ///< Ошибка реконструкции : $||(A - A_{reconstructed})||_{p}$
                  ///< (абсолютная) или $|| (A - A_{reconstructed}) ./ A ||_{p}$
                  ///< (относительная)
    MAX_DEVIATION  ///< Максимальное из отклонений U и V (использующих норму
                   ///< Lp).
  };
  ...
  struct MetricSettings {
    ...
    /**
     * @brief Оператор сравнения для использования в ассоциативных контейнерах
     * (например std::map).
     */
  ...
  struct svd_test_funcSettings {
    ...
    std::vector<FloatingPoint>
        SigmaMaxMinRatiosVec;  ///< Коэффициенты sigma_max/sigma_min. Чем
                               ///< больше, тем хуже обусловленность, 1 = все
                               ///< сигмы равны.
    std::vector<std::pair<int, int>>
        MatSizesVec;  ///< Размеры тестируемых матриц.
    int n;            ///< Количество итераций (выборка).
    std::string algorithmName;  ///< Название алгоритма SVD для тестирования
                                ///< (должно соответствовать имени,
                                ///< зарегистрированному в `algorithmsInfo`).
  ...
  /**
   * @brief Конструктор, который немедленно запускает тесты на основе
   * предоставленных настроек.
   ...
   * этот метод является внутренней частью тестового фреймворка и обычно
   * вызывается из метода run_tests_parallel.
   *
   ...
   *
   * Формула нормы:
   * $$||M||_{p,q} = \left( \sum_{i=1}^m \left( \sum_{j=1}^n |M_{ij}|^p
   * \right)^{q/p} \right)^{1/q}$$
   *
  ...
  /**
   * @brief Сравнивает исходную матрицу с реконструированной матрицей из
   * результатов SVD.
   *
   * Выводит исходную и реконструированную матрицы, а также процент совпадения
   * знаков элементов.
   *
   * @param algoName Имя использованного алгоритма.
   * @param rows Количество строк в матрице.
   * @param cols Количество столбцов в матрице.
   * @param computationOptions Опции, использованные для вычисления SVD. По
   * умолчанию 0.
   * @param out Выходной поток для результатов и деталей сравнения. По умолчанию
   * null_stream.
   */
 ...
 protected:
  ...
  /**
   * @brief Функциональный объект для выполнения SVD-разложения конкретным
   * алгоритмом.
   *
   * Принимает матрицу, опции, необязательный поток для вывода отладки и
   * необязательный вектор истинных сингулярных значений. Возвращает структуру
   * SVDResult.
   */
  ...
  /**
   * @brief Запускает один тест SVD для конкретного алгоритма и подробных
   * настроек.
   *
   ...
   * @param divergence_stream Поток для вывода отладочной информации алгоритмом
   * (опционально).
   * @param true_singular_values Указатель на вектор истинных сингулярных
   * значений (опционально).
  ...
  /**
   * @brief Печатает двумерный вектор строк в виде форматированной таблицы в
   * указанный поток вывода.
  ...
  /**
   * @brief Печатает двумерный вектор строк в формате CSV в указанный поток
   * вывода.
   * @param out Выходной поток.
   ...
   *
   * Инициализируется при запуске программы и используется для поиска и вызова
   * алгоритмов по их имени.
   */
