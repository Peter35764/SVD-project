...
SVDClass create_svd_impl(const Matrix &A, const Vector &sigma, unsigned int options,
                         bool solve_with_sigmas, std::true_type ) {
    if (solve_with_sigmas) {
      return SVDClass(A, sigma, options);
    } else {
       Eigen::JacobiSVD<Matrix> svd_ref(A, Eigen::ComputeThinU | Eigen::ComputeThinV);
...
SVDClass create_svd(const Matrix &A, const Vector &sigma, unsigned int options,
    ...
    return create_svd_impl<SVDClass>(A, sigma, options, solve_with_sigmas, requires_sigma<SVDClass>{});
}
...
template <typename FloatingPoint, typename MatrixType>
std::map<std::string, typename SVD_Test<FloatingPoint, MatrixType>::SvdRunnerFunc>
SVD_Test<FloatingPoint, MatrixType>::initialize_svd_runners() {
    std::map<std::string, SvdRunnerFunc> runners;

    runners["SVD_Project::GivRef_SVD"] = [](SVD_Test* instance, const svd_test_funcSettings& s) {
        instance->template svd_test_func<SVDGenerator, SVD_Project::GivRef_SVD>(s);
    };
    runners["SVD_Project::v0_GivRef_SVD"] = [](SVD_Test* instance, const svd_test_funcSettings& s) {
        instance->template svd_test_func<SVDGenerator, SVD_Project::v0_GivRef_SVD>(s);
    };
     runners["SVD_Project::NaiveMRRR_SVD"] = [](SVD_Test* instance, const svd_test_funcSettings& s) {
        instance->template svd_test_func<SVDGenerator, SVD_Project::NaiveMRRR_SVD>(s);
    };
     runners["SVD_Project::v0_NaiveMRRR_SVD"] = [](SVD_Test* instance, const svd_test_funcSettings& s) {
        instance->template svd_test_func<SVDGenerator, SVD_Project::v0_NaiveMRRR_SVD>(s);
    };
     runners["SVD_Project::RevJac_SVD"] = [](SVD_Test* instance, const svd_test_funcSettings& s) {
        instance->template svd_test_func<SVDGenerator, SVD_Project::RevJac_SVD>(s);
    };
     runners["SVD_Project::v0_RevJac_SVD"] = [](SVD_Test* instance, const svd_test_funcSettings& s) {
        instance->template svd_test_func<SVDGenerator, SVD_Project::v0_RevJac_SVD>(s);
    };
     runners["Eigen::JacobiSVD"] = [](SVD_Test* instance, const svd_test_funcSettings& s) {
...
template <typename FloatingPoint, typename MatrixType>
std::map<std::string, typename SVD_Test<FloatingPoint, MatrixType>::SvdRunnerFunc>
SVD_Test<FloatingPoint, MatrixType>::svd_test_runners = initialize_svd_runners();
...
void SVD_Test<FloatingPoint, MatrixType>::run_tests_parallel(
    ...
    threads.emplace_back([this, s, &test_times, &dur_mutex, &sem]() {
      ...
      if (it != svd_test_runners.end()) {
      ...
      } else {
          {
              ...
              std::cerr << "\nERROR: Unknown algorithm '" << s.algorithmName << "' in run_tests_parallel.\n";
          }
      }
  ...
  std::cout << "\033[" << output_line << ";0H"
    ...
    if (vec_settings.empty()) {
        std::lock_guard<std::mutex> lock(cout_mutex);
        std::cerr << "Error: Test settings vector is empty, cannot write times file.\n";
        return;
    }
...
void SVD_Test<FloatingPoint, MatrixType>::svd_test_func(
  ...
  for (const auto &MatSize : MatSizesVec) {
    ...
    for (const auto &SigmaMaxMinRatio : SigmaMaxMinRatiosVec) {
      for (const auto &interval : Intervals) {
        ...
        assert((SigmaMaxMinRatio >= FloatingPoint(1)) && "Error: Sigma ratio must be >= 1");

         if (interval.second < interval.first * SigmaMaxMinRatio && interval.first != 0) {
             continue;
         }

         FloatingPoint sigma_min_lower_bound = interval.first;
         FloatingPoint sigma_min_upper_bound = (SigmaMaxMinRatio > 0) ? (interval.second / SigmaMaxMinRatio) : interval.second;
         if (sigma_min_upper_bound < sigma_min_lower_bound && sigma_min_lower_bound > 0) {
            sigma_min_upper_bound = sigma_min_lower_bound;
         }

        std::uniform_real_distribution<FloatingPoint> distrSigmaMin(sigma_min_lower_bound, sigma_min_upper_bound);
        ...
        for (size_t i = 1; i <= n; ++i) {
          ...
          assert((minNM >= 1) && "Error: Matrix dimensions must be at least 1x1");
        ...
        row.push_back(std::to_string(N) + "x" + std::to_string(M)); // Dimension как строка
  ...
  if (csv_file) {
  ...
  }
    {
        ...
        progressStream << "]";
        if (lineNumber > 0) {
             std::cout << "\033[" << lineNumber << ";0H" << progressStream.str() << "\033[K" << std::endl;
        } else {
...
template <typename FloatingPoint, typename MatrixType>
std::map<std::string, typename SVD_Test<FloatingPoint, MatrixType>::SvdExecutorFunc>
SVD_Test<FloatingPoint, MatrixType>::initialize_svd_executors() {
    std::map<std::string, SvdExecutorFunc> executors;

    auto create_executor = []<template<typename> class SvdImpl>(const MatrixDynamic& A, unsigned int options) -> SVDResult {
        ...
        auto svd = create_svd<SvdImpl<MatrixDynamic>>(A, sigma_to_pass, options, needs_sigma);

        return {svd.matrixU(), svd.singularValues(), svd.matrixV()};
    };

    executors["Eigen::JacobiSVD"] = [](const MatrixDynamic& A, unsigned int options) -> SVDResult {
         Eigen::JacobiSVD<MatrixDynamic> svd(A, options);
         return {svd.matrixU(), svd.singularValues(), svd.matrixV()};
    };

    executors["SVD_Project::GivRef_SVD"] = [&create_executor](const MatrixDynamic& A, unsigned int options) -> SVDResult {
        return create_executor.template operator()<SVD_Project::GivRef_SVD>(A, options);
    };
    executors["SVD_Project::v0_GivRef_SVD"] = [&create_executor](const MatrixDynamic& A, unsigned int options) -> SVDResult {
        return create_executor.template operator()<SVD_Project::v0_GivRef_SVD>(A, options);
    };
    executors["SVD_Project::NaiveMRRR_SVD"] = [&create_executor](const MatrixDynamic& A, unsigned int options) -> SVDResult {
        return create_executor.template operator()<SVD_Project::NaiveMRRR_SVD>(A, options);
    };
    executors["SVD_Project::v0_NaiveMRRR_SVD"] = [&create_executor](const MatrixDynamic& A, unsigned int options) -> SVDResult {
        return create_executor.template operator()<SVD_Project::v0_NaiveMRRR_SVD>(A, options);
    };
    executors["SVD_Project::RevJac_SVD"] = [&create_executor](const MatrixDynamic& A, unsigned int options) -> SVDResult {
        return create_executor.template operator()<SVD_Project::RevJac_SVD>(A, options);
    };
    executors["SVD_Project::v0_RevJac_SVD"] = [&create_executor](const MatrixDynamic& A, unsigned int options) -> SVDResult {
        return create_executor.template operator()<SVD_Project::v0_RevJac_SVD>(A, options);
    };
...
template <typename FloatingPoint, typename MatrixType>
std::map<std::string, typename SVD_Test<FloatingPoint, MatrixType>::SvdExecutorFunc>
SVD_Test<FloatingPoint, MatrixType>::svd_executors = initialize_svd_executors();


template <typename FloatingPoint, typename MatrixType>
typename SVD_Test<FloatingPoint, MatrixType>::SVDResult
SVD_Test<FloatingPoint, MatrixType>::execute_svd_algorithm(const std::string& algoName, const MatrixDynamic& A, unsigned int options)
{
    auto it = svd_executors.find(algoName);
    if (it != svd_executors.end()) {
        return it->second(A, options);
    } else {
        throw std::invalid_argument("Unknown algorithm name in execute_svd_algorithm: " + algoName);
    }
}
...
void SVD_Test<FloatingPoint, MatrixType>::compareMatrices(const std::string &algoName, int rows, int cols, std::ostream &out) {
  ...
  SVDResult result = execute_svd_algorithm(algoName, A, Eigen::ComputeFullU | Eigen::ComputeFullV);
  ...
  FloatingPoint percent = (total > 0) ? (100.0 * static_cast<FloatingPoint>(count) / static_cast<FloatingPoint>(total)) : 0.0;

  out << "Algorithm: " << algoName << "\n";
  out << "Original Matrix (" << rows << "x" << cols << "):\n" << A << "\n\n";
  out << "Reconstructed Matrix:\n" << A_rec << "\n\n";
  out << "Percentage of matching signs (based on all elements): " << std::fixed << std::setprecision(2) << percent << "%\n";
...
FloatingPoint SVD_Test<FloatingPoint, MatrixType>::count_metrics(
    ...
    const MatrixDynamic &S_true_mat)
{
  ...
  switch (metric_settings.type) {
    case U_DEVIATION1:
      ans = Lp_norm((IdU - U_calc * U_calc.transpose()).eval(), metric_settings.p);
      if (metric_settings.is_relative && norm_IdU != 0) {
        ans /= norm_IdU;
      }
      break;
    case U_DEVIATION2:
      ans = Lp_norm((IdU - U_calc.transpose() * U_calc).eval(), metric_settings.p);
       if (metric_settings.is_relative && norm_IdU != 0) {
        ans /= norm_IdU;
      }
      break;
    case V_DEVIATION1:
      ans = Lp_norm((IdV - V_calc * V_calc.transpose()).eval(), metric_settings.p);
       if (metric_settings.is_relative && norm_IdV != 0) {
        ans /= norm_IdV;
      }
      break;
    case V_DEVIATION2:
       ans = Lp_norm((IdV - V_calc.transpose() * V_calc).eval(), metric_settings.p);
       if (metric_settings.is_relative && norm_IdV != 0) {
        ans /= norm_IdV;
      }
      break;
    case ERROR_SIGMA:
       {
            ...
            if (size_to_compare > 0) {
                VectorDynamic abs_err = (S_true_vec.head(size_to_compare) - S_calc.head(size_to_compare)).cwiseAbs();
                if (metric_settings.is_relative) {
                    ...
                    for(int k=0; k<size_to_compare; ++k) {
                        ...
                        if (std::abs(S_true_vec(k)) > std::numeric_limits<FloatingPoint>::epsilon()) { // Используем эпсилон для сравнения с 0
                            rel_err(k) = abs_err(k) / std::abs(S_true_vec(k));
                        } else if (std::abs(abs_err(k)) > std::numeric_limits<FloatingPoint>::epsilon()) {
                            // Если истинное значение 0, а ошибка не 0, относительная ошибка бесконечна или велика
                             rel_err(k) = std::numeric_limits<FloatingPoint>::infinity(); // Или большое число
                        } else {
                             // Если и истинное значение, и ошибка близки к 0, относительная ошибка 0
                             rel_err(k) = 0.0;
                        }
                    }
      ...
      if (metric_settings.is_relative) {
        ans = std::max(
            {count_metrics({U_DEVIATION1, metric_settings.p, true, "", false},
                           Usize, Vsize, U_calc, V_calc, S_calc, U_true, V_true, S_true_mat),
  ...
  if (std::isnan(ans) || std::isinf(ans)) {
     // Обработка: вернуть 0, максимальное значение или другое значение по умолчанию
