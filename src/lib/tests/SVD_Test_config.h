 ...
 * @brief Содержит конфигурационные настройки и регистрацию алгоритмов для SVD_Test.
 *
...
/**
 * @brief Трейт для определения, требует ли конструктор алгоритма SVD передачи истинных сингулярных значений.
...
/**
 * @brief Специализация для RevJac_SVD, указывающая, что конструктор этого алгоритма может требовать спектр.
...
/**
 * @brief Специализация для v0_RevJac_SVD, указывающая, что конструктор этого алгоритма может требовать спектр.
 * @tparam Matrix Тип матрицы.
 */
template <typename Matrix>
 ...
 *
 * Этот статический вектор содержит список всех алгоритмов SVD, доступных для тестирования.
 * Каждый элемент вектора создается с помощью статического метода `createAlgorithmInfoEntry`,
 * который связывает шаблонный класс алгоритма SVD с его строковым именем и функциональными
 * объектами для запуска тестов и выполнения разложения.
 *
 * Чтобы добавить новый алгоритм SVD в тестовый фреймворк, выполните следующие шаги:
 * 1. **Реализуйте класс вашего алгоритма SVD.** Ваш класс должен быть шаблонным по типу данных
 * и иметь публичные методы для получения результатов SVD:
 * - `const MatrixDynamic& matrixU() const;` (или аналогичный, возвращающий левые сингулярные векторы)
 * - `const VectorDynamic& singularValues() const;` (или аналогичный, возвращающий сингулярные значения)
 * - `const MatrixDynamic& matrixV() const;` (или аналогичный, возвращающий правые сингулярные векторы)
 * Класс также должен иметь конструктор(ы), принимающий(е) входную матрицу для разложения
 * и опции вычисления (например, `unsigned int computationOptions`). Опционально могут
 * поддерживаться конструкторы с `std::ostream*` для вывода отладки и с вектором истинных
 * сингулярных значений (если алгоритм их использует).
 * 2. Если ваш алгоритм требует передачи истинных сингулярных значений в конструктор,
 * добавьте специализацию трейта `requires_sigma` (см. выше).
 * 3. Добавьте вызов `createAlgorithmInfoEntry<YourNewSVDAlgorithm>("YourNewAlgorithmName")`
 * в список инициализации этого вектора.
 *
 * @code{.cpp}
 ...
 * createAlgorithmInfoEntry<YourNewSVDAlgorithm>("YourNewAlgorithmName"), // <-- Добавьте здесь вашу запись
 ...
 * которое будет использоваться в настройках тестов (`svd_test_funcSettings::algorithmName`).
 */
