...
### SVDCyclicGetCyclicMat(SVD svd,Mat A,Mat AT,Mat *C) - функция предназначена для вычисления циклической матрицы 
𝐶 на основе переданных параметров A и $A^T$. Поддерижвается как CUDA и HIP (параллельное распределение процессов), так CUDA и HIP одновременно.
