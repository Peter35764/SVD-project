...
Важные определения:
*Подпространство Крылова - линейное пространство, порождённое вектором v &in; C <sup>n</sup> и матрицей A &in;
$\[
C^{n \times n}
\]$
...
где $\(\alpha_j = v_j^* A v_j\)$ и $\(\beta_{j+1} = v_{j+1}^* A v_j\), $что порождает ортонормированный набор векторов Ланцоша <i>v<sub>j</sub></i> и тридиагональную матрицу, определённую как
